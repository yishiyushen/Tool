...
public class ThreeDSlidingLayout extends RelativeLayout implements OnTouchListener {
	...
	public ThreeDSlidingLayout(Context context, AttributeSet attrs) {
		...
		image3dView = new Image3DView(context);
	}
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (changed && !loadOnce) {
			// 获取左侧布局对象
			leftLayout = getChildAt(0);//findViewById(R.id.menu);
			leftLayoutParams = (MarginLayoutParams) leftLayout.getLayoutParams();
			rightEdge = -leftLayoutParams.width;
			// 获取右侧布局对象
			rightLayout = getChildAt(1);//findViewById(R.id.content);
			...
			addViewInLayout(image3dView, 2, params);
